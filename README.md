...
│   ├── extract_selected_features.py   # Generate *_selected_features.csv from saved models
...
### 3 — Generate features CSV files

After training, run this once to produce a `*_selected_features.csv` file alongside each model:

```bash
python ml/extract_selected_features.py
```

Each CSV lists the features (in SHAP importance order) that the corresponding pipeline expects. These files are required by `inference.py` and the desktop application. Pre-generated CSVs are included in the repository under `ml/outputs/saved_models/`.
...
### 6 — Single-scenario inference (programmatic)
